...
All scripts use a venv at `~/.agents/skills/pdf-reader/.venv` with `pymupdf` installed. If the venv is missing, create it from `requirements.txt`:

```bash
python3 -m venv ~/.agents/skills/pdf-reader/.venv
~/.agents/skills/pdf-reader/.venv/bin/pip install -r ~/.agents/skills/pdf-reader/requirements.txt
```
...
All scripts are in `~/.agents/skills/pdf-reader/scripts/`.
