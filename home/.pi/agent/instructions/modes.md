...
In ask/brainstorm/plan, `bash` is replaced by `bash_readonly`. In ask/brainstorm
only, mutating file writes are further restricted to markdown (plus `/tmp` and
`~/.pi`); plan writes are unfiltered. A blocked call's reason names the active mode.
The mode can change mid-run. `mode_status` reports the live mode and tool-gating
state — consult it per its own usage guidance. Never fight a gate; instead tell the
